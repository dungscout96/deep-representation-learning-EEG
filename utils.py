...
def check_accuracy(loader, set_type, model, device, dtype, logger):
    '''
...
def train(model, loader_train, optimizer, loader_val, epochs, logger, device, dtype):
    ...
    for e in range(epochs):
        ...
        val_acc = check_accuracy(loader_val, 'val', model, device, dtype, logger)
...
def run_experiment(seed, loader_train, loader_val, model_name, feature, num_epoch, logger, device, dtype):
    model = create_model(model_name, feature)
    ...
    model = train(model, loader_train=loader_train, optimizer=optimizer, loader_val=loader_val, epochs=num_epoch, logger=logger, device=device, dtype=dtype)
...
def test_model(model, test_data, subj_csv, device, dtype, logger):
    ...
    per_sample_acc = check_accuracy(loader_test, 'test', model, device, dtype, logger)
